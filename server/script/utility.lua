...
function m.setTypeParent(obj, parent, mark)
    mark = mark or {}
    if mark[obj] then
        return
    end
    mark[obj] = true
    obj.parent = parent
    if obj.type == "type.library"
    or obj.type == "type.field"
    or obj.type == "type.index"
    or obj.type == "type.variadic"
    or obj.type == "type.alias" then
        m.setTypeParent(obj.value, obj, mark)
    end
    if obj.type == "type.list"
    or obj.type == "type.table"
    or obj.type == "type.union"
    or obj.type == "type.inter"
    or obj.type == "type.generics" then
    ...
    or     obj.type == "type.field" then
        m.setTypeParent(obj.key, obj, mark)
    elseif obj.type == "type.function" then
        m.setTypeParent(obj.args, obj, mark)
        m.setTypeParent(obj.returns, obj, mark)
    end
    if obj.generics then
        m.setTypeParent(obj.generics, obj, mark)
    end
    if obj.type == "type.alias" then
        m.setTypeParent(obj.name, obj, mark)
    end
    if obj.child then
        for _, value in pairs(obj.child) do
            m.setTypeParent(value, obj, mark)
        end
    end
end
