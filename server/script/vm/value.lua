...
local function addAeroService(value)
    local source = value:getSource()
    if not (source and source.type == "name") then
        return
    end
    for _, libName in pairs{"AeroService", "AeroController"} do
        local lib = library.object[libName]
        if lib then
...
local function addAeroController(value)
    local source = value:getSource()
    if not (source and source.type == "name") then
        return
    end
    local lib = library.object.AeroController
    if lib then
        local childs = libraryBuilder.child(lib)
...
function mt:fixType(type)
    if type and type:sub(1, 5) == "Enum." then
        return "EnumItem"
    end
    return type
end
...
function mt:getLibChild(index)
    local tp = self:fixType(self:getType())
...
function mt:getChild(index, source, uri)
    -- self:setType('table', 0.5)
    local parent = self
    local value
    -- 最多检查3层 __index
    for _ = 1, 3 do
        value = parent:rawGet(index, uri)
        if value then
            break
        end
...
function mt:setEmmy(emmy)
    ...
    elseif emmy.type == 'emmy.type' then
        ...
        if emmyType.name == "AeroService" then
            addAeroService(self)
        elseif emmyType.name == "AeroController" then
            addAeroController(self)
        end
