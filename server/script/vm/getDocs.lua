...
local function insertLibraryTypes(name, results)
    for objName in pairs(rbxlibs.object) do
        if name == "*" or objName == name then
            results[#results+1] = {
                [1] = objName,
                type = "doc.library"
            }
...
local function getDocTypes(name)
    ...
    insertLibraryTypes(name, results)
    return results
end
