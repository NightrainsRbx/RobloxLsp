...
local function copyValue(value)
    ...
    local type = {}
    for k, v in pairs(value._type) do
        type[k] = v
    end
    copy._type = type
    copy._copy = true
...
local function shouldCopy(func, value)
    if (not func.start) or (not func.finish)
    or func.start == 0 or func.finish == 0
    or value._info._count == 0
    or not value._global
    then
        return true
    end
    return false
...
function mt:getReturnCopy(index)
    local returns = self:getReturn()
    if returns then
        local func = self:getSource()
        local returnsCopy = createMulti()
        returns:eachValue(function(index, value)
            if value:getSource() then
                local copy = shouldCopy(func, value)
                if copy and func.start and func.finish and func.start ~= 0 then
                    value:eachInfo(function(info, source)
                        if not copy then
                            return
                        end
                        if source.start < func.start or source.finish > func.finish then
                            copy = false
                        end
                    end)
                end
                value = copy and copyValue(value) or value
            end
            returnsCopy:set(index, value)
        end)
        if index then
            return returnsCopy:get(index)
        else
            return returnsCopy
        end
    end
