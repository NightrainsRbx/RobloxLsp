...
function mt:buildTable(source)
    ...
    for index, obj in ipairs(source) do
        ...
        if obj.type == 'pair' then
            ...
            if value then
                if value._copy then
                    ...
                    local copy = self:createValue(value:getType(), source)
                    value = self:inheritValue(copy, value)
                end
                local key   = obj[1]
        ...
        elseif obj.type:sub(1, 4) == 'emmy' then
            ...
            if obj.type == "..." and value:getEmmy() then
                local vararg = value:getEmmy()
                local emmyArray = self:buildEmmyArrayType({
                    type = "emmyArrayType",
                    start = source.start,
...
function mt:callRequire(func, values)
    ...
    local strSource = value:getSource()
    if not strSource then
        return nil
    end
...
function mt:call(value, values, source, copy)
            ...
            if not source:get 'called' then
                ...
                for i, v in ipairs(values) do
                    if v._copy then
                        ...
                        local copy = self:createValue(v:getType(), source)
                        copy._copy = true
                        values[i] = self:inheritValue(copy, v)
                    end
                end
                func:setArgs(values)
    ...
    local typedReturns = self:doTypedFunction(source)
    if typedReturns then
        return typedReturns
    end
    if copy then
        return func:getReturnCopy()
    end
...
function mt:inheritValue(value, from)
    value._literal = from._literal
    value._info = from._info
    value._child = from._child
    value._lib = from._lib
    value._emmy = from._emmy
    return value
end
...
function mt:getName(name, source)
    ...
    if not loc and name == "script" and config.isLuau() then
        local scriptValue = self:findScriptByPath(self.uri)
        if not scriptValue then
            scriptValue = self:createValue("LuaSourceContainer", source)
        end
        loc = self:createLocal(name, source, scriptValue)
    end
    if loc then
    ...
    global = ENVValue:getChild(name, source, source.uri)
...
function mt:getSimple(simple, max)
    ...
    for i = 2, max do
        ...
        if source.type == 'call' then
            ...
            value = self:call(func, values, source, true) or valueMgr.create('any', self:getDefaultSource())
        elseif source.type == 'index' then
            local child = source[1]
            local index = self:getIndex(source)
            child:set('parent', value)
            value:addInfo('get child', source, index)
            value = value:getChild(index, source, source.uri)
        ...
        elseif source.type == 'name' then
            ...
            value = value:getChild(source[1], source, source.uri)
            if value and value._lib and value._lib.special == "aeroModules" then
                self:searchAeroModules(value)
            end
            source:bindValue(value, 'get')
...
function mt:getBinary(exp)
    ...
    local op = exp.op
    local rbxType = self:getRbxBinaryExp(v1, v2, op)
    if rbxType then
        return self:createValue(rbxType, exp)
    end
    if     op == 'or' then
...
function mt:setOne(var, value, emmy, comment)
    if not value then
        ...
        if value._copy then
            ...
            local copy = valueMgr.create(value:getType(), self:getDefaultSource())
            value = self:inheritValue(copy, value)
        end
    end
    value:setEmmy(emmy)
...
local function compile(vm, ast, lsp, uri)
    ...
    ast.uri = vm.uri
    -- 根据运行版本决定环境实现方式
    if config.config.runtime.version == 'Lua 5.1' or config.config.runtime.version == 'LuaJIT' or config.config.runtime.version == 'Luau' then
        vm.envType = 'fenv'
    else
        vm.envType = '_ENV'
    end
...
local function concatOptions(options)
    if not options then
        return ""
    end
    local str = ""
