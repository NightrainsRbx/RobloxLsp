...
local function addIgnoreMember(name, uri, callback)
    callback {
        ...
        kind = 'quickfix',
        command = {
            title = lang.script.COMMAND_IGNORE_NAME,
            command = 'lua.config',
            arguments = {
                {
...
local function solveDeprecatedRbxMember(lsp, uri, data, callback)
    ...
    local finish = lines:position(data.range['end'].line + 1, data.range['end'].character)
    local name = text:sub(start, finish)
    if rbxApi.DeprecatedQuickFix[name] then
        callback {
            title = lang.script('ACTION_FIX_DEP_MEMBER', rbxApi.DeprecatedQuickFix[name]),
            kind = 'quickfix',
            edit = {
                changes = {
                    [uri] = {
                        {
                            ...
                            newText = rbxApi.DeprecatedQuickFix[name],
                        }
                    }
                }
            }
...
local function solveDiagnostic(lsp, uri, data, callback)
    ...
    if data.code == 'undefined-rbx-member' then
        solveUndefinedRbxMember(lsp, uri, data, callback)
    end
    if data.code == 'deprecated-rbx-member' then
        solveDeprecatedRbxMember(lsp, uri, data, callback)
    end
