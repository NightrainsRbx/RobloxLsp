...
local function insertChild(tbl, name, key, value, type)
...
local function mergeLibs(alllibs, libs, libName)
    ...
    for _, lib in pairs(libs) do
        if lib.type == 'class' then
            rbxApi.NewTypes[lib.name] = true
        end
        if lib.parent then
...
local function initRbxTypes(rbxLibs)
    for parent, members in pairs(rbxLibs.objects) do
        Library.object[parent] = {
            type = parent,
            name = parent,
            child = {},
        }
        for name, lib in pairs(members) do
            insertChild(Library.object, parent, name, lib)
        end
    end
end
