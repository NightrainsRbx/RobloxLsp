...
local function buildTypeAlias(source, text, used, symbols)
    symbols[#symbols+1] = {
        name           = source.name[1],
        detail         = ('type %s'):format(source.generics and guide.buildTypeAnn(source.generics) or ""),
        kind           = define.SymbolKind.Class,
        range          = { source.start, source.finish },
        selectionRange = { source.start, source.finish },
        valueRange     = { source.value.start, source.value.finish },
...
local function buildSource(source, text, used, symbols)
    ...
    elseif source.type == 'type.alias' then
        buildTypeAlias(source, text, used, symbols)
    end
end
