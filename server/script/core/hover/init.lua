...
local function getHoverAsValue(source)
	local oop = source.type == "method" or source.type == "getmethod" or source.type == "setmethod"
...
local function getHoverAsFunction(source, oop)
	...
	for _, def in ipairs(values) do
		...
		elseif def.type == "table" or def.type == "boolean" or def.type == "string" or def.type == "number" then
			other = other + 1
