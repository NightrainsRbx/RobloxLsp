...
local function asTypeAnn(typeAnn)
    local returns = {}
    if typeAnn.type == "type.list" then
        for i = 1, #typeAnn do
            returns[#returns+1] = guide.buildTypeAnn(typeAnn[i])
        end
    else
        returns[1] = guide.buildTypeAnn(typeAnn)
    end
    if #returns == 0 then
        return
    end
...
local function asFunction(source)
    if source.returnTypeAnn then
        return asTypeAnn(source.returnTypeAnn.value)
    end
    ...
    for i, rtn in ipairs(dual) do
        ...
        for n = 1, #rtn do
            local values = vm.getInfers(rtn[n], 0)
...
return function (source)
    ...
    if source.type == "type.function" then
        return asTypeAnn(source.returns)
    end
end
