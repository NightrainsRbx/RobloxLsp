...
function mt:addParam(source, bind)
    ...
    if getmetatable(bind).type == 'emmy.generic' then
...
return function ()
    ...
    if config.isLuau() then
        self:newClass('Objects',    'any',    source)
        self:newClass('Array',      'any',    source)
        self:newClass('Map',        'any',    source)
        self:newClass('Dictionary', 'any',    source)
        self:newClass('Tuple', 'any',    source)
        for rbxClass in pairs(rbxApi:getTypes()) do
            self:newClass(rbxClass, nil, source)
        end
