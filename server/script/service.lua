...
function mt:_doCompileTask()
    ...
    local fastAutocompletion = config.config.completion.fastAutocompletion
    while true do
        if fastAutocompletion then
            self:_loadProto()
        end
...
function mt:onUpdateConfig(updated, other)
    ...
    if newConfig.workspace.rojoProjectFile ~= oldConfig.workspace.rojoProjectFile then
